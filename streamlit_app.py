...
def save_as_pdf(sheet, page_ranges, output_folder, file_month):
    ...
    for idx, cell_range in page_ranges.items():
        ...
        supplier_idx = headers.index('SUPPLIER') if 'SUPPLIER' in headers else None
        if data.empty:
            print("Data is unexpectedly empty before file name creation.")
            print(cell_range)
            file_name = f"{idx}.pdf"
        else:
            file_name = (f"{sanitize_filename(data.iloc[0, supplier_idx])}_{file_month}.pdf"
                            if month_idx is not None and supplier_idx is not None 
                            else f"{idx}.pdf"
                            )
        ...
        for row_idx, row in data.iterrows():
            ...
            for j, item in enumerate(row):
                ...
                if headers[j] in ['GL Value', 'UNIT COST'] and item is not None and not pd.isna(item):
                        try:
...
def main():
    ...
    st.title("Samples Invoice Backups")
    uploaded_file = st.file_uploader("Upload your Excel file (Updated October 2025) ", type=["xlsx"])
    ...
    if uploaded_file is not None:
        ...
        file_month = uploaded_file.name[-13:-5]
        ...
        try:
            ...
            pdf_files = save_as_pdf(sheet, page_ranges, "generated_pdfs", file_month)
